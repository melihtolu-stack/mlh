...
export default function ProductDetailClient({
  ...
  const [quantity, setQuantity] = useState(minimumOrderQuantity || 100)
  const [message, setMessage] = useState<string | null>(null)

  const handleAddToBasket = () => {
    try {
      const stored = typeof window !== "undefined" ? window.localStorage.getItem(BASKET_STORAGE_KEY) : null
      const parsed = stored ? JSON.parse(stored) : []
      const basket = Array.isArray(parsed) ? parsed : []
      const safeQuantity = Math.max(minimumOrderQuantity || 100, quantity || 0)
      const exists = basket.find((item: { productId: string }) => item.productId === productId)
      const updated = exists
        ? basket.map((item: { productId: string; quantity: number }) =>
            item.productId === productId ? { ...item, quantity: safeQuantity } : item
          )
        : [...basket, { productId, quantity: safeQuantity }]
      window.localStorage.setItem(BASKET_STORAGE_KEY, JSON.stringify(updated))
      setMessage("Added to quote basket.")
    } catch (err) {
      console.error("Failed to update basket:", err)
  ...
  return (
    <div className="space-y-4">
      <div>
        ...
        <input
          type="number"
          min={minimumOrderQuantity || 100}
          value={quantity}
          onChange={(event) => setQuantity(Number(event.target.value) || 0)}
          className="mt-2 w-full rounded-xl border border-gray-200 px-4 py-2.5 text-sm focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none"
        />
        <p className="text-xs text-gray-400 mt-2">Minimum order: {minimumOrderQuantity || 100}</p>
      </div>
